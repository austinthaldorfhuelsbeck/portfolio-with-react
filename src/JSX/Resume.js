import React from "react";
import CategoryHeader from "../Components/CategorySection/CategoryHeader/CategoryHeader";

export default function Resume() {
  return (
    <div className="project-content">
      <CategoryHeader title="Resume" />
      <section className="category-section" id="employment">
        <header className="category-header" data-aos="fade-up">
          <h1 className="category-title">Employment</h1>
          <hr />
        </header>

        <div data-aos="fade-up">
          <h2>The Seattle School of Theology &amp; Psychology</h2>
          <h3>Multimedia Specialist</h3>
          <p>Digital Marketing | Video Editing | Podcast Editing</p>
          <p>
            <a
              title="The Seattle School"
              href="/AustinThaldorfHuelsbeck-projects/theseattleschool.html"
            >
              <strong>View Details &rarr;</strong>
            </a>
          </p>
          <p>2018&ndash;</p>
        </div>

        <div data-aos="fade-up">
          <h2>Austin + Emily Creative</h2>
          <h3>Wedding Videography</h3>
          <p>Business Owner | Videographer | Video Editing</p>
          <p>
            <a
              title="Austin + Emily Creative"
              href="https://austinandemilycreative.com/films"
              target="_blank"
            >
              <strong>View Samples &rarr;</strong>
            </a>
          </p>
          <p>2018&ndash;</p>
        </div>

        <div data-aos="fade-up">
          <h2>Zero To Sixty by AGD</h2>
          <h3>Creative Director</h3>
          <p>Videographer | Video Editing | Animation | Graphic Design</p>
          <p>
            <a
              title="Zero To Sixty"
              href="https://agdentertainment.com/about-zero-to-60"
              target="_blank"
            >
              <strong>View Website &rarr;</strong>
            </a>
            <br />
            <a
              title="AGD Reel"
              href="https://vimeo.com/294708444"
              target="_blank"
            >
              <strong>View Reel &rarr;</strong>
            </a>
          </p>
          <p>2018&ndash;2019</p>
        </div>

        <div data-aos="fade-up">
          <h2>Worship Online</h2>
          <h3>Digital Marketing</h3>
          <p>
            Videographer | Video Editing | Podcast Editing | Marketing Campaigns
            &amp; Strategy
          </p>
          <p>
            <a
              title="Worship Online"
              href="http://worshiponline.com"
              target="_blank"
            >
              <strong>View Website &rarr;</strong>
            </a>
          </p>
        </div>
      </section>

      <section className="category-section" id="skills" data-aos="fade-up">
        <header className="category-header">
          <h1 className="category-title">Skills</h1>
          <hr />
        </header>

        <h2 data-aos="fade-up">Development</h2>
        <p data-aos="fade-up">
          I am currently enrolled in{" "}
          <a
            href="https://www.thinkful.com/bootcamp/web-development/flexible/"
            target="_blank"
          >
            <strong>Thinkful</strong>
          </a>
          's Flex Engineering program, where I am learning to program websites
          like the one you are reading in <strong>HTML</strong> &amp;{" "}
          <strong>CSS</strong>.
        </p>
        <p data-aos="fade-up">
          I am also learning to program in <strong>JavaScript</strong> with
          <strong>Node.js</strong> &amp; <strong>React</strong>. These
          technologies form the base of my daily approach to development.
        </p>
        <p data-aos="fade-up">
          For server-side development, I am learning the{" "}
          <strong>Express</strong> framework. I am passionate about both
          client-side and server-side development, and it is important to me to
          be well-versed in both.
        </p>
        <p data-aos="fade-up">
          I use <strong>Git</strong> &amp; <strong>GitHub</strong> to manage
          revisions for all of my projects. Check out my{" "}
          <a href="https://github.com/austinthaldorfhuelsbeck" target="_blank">
            <strong>GitHub page</strong>
          </a>{" "}
          to see my most recent projects and contributions.
        </p>
        <p data-aos="fade-up">
          My preferred code editor is <strong>VSCode</strong>. I am quite
          comfortable working in both <strong>Windows</strong> and{" "}
          <strong>OS X</strong>, with basic knowledge of
          <strong>Linux</strong>.
        </p>

        <h2 data-aos="fade-up">Video &amp; Design</h2>
        <p data-aos="fade-up">
          I have been shooting and editing video professionally for over six
          years. From freelance, to startups, to an established institution, I
          have been able to work with a wide variety of clients and deliver
          results on-time.
        </p>
        <p data-aos="fade-up">
          Beyond communication and professionalism, this experience has also
          allowed me to develop strong skills with{" "}
          <strong>Adobe Creative Cloud</strong>, including Photoshop, Premiere
          Pro, Illustrator, Lightroom, and more.
        </p>
        <p data-aos="fade-up">
          I have also gained real-world experience designing graphics and video
          for the web, managing content using tools like <strong>Libsyn</strong>{" "}
          &amp; <strong>Wordpress</strong>. I have been able to lead and
          contribute to digital marketing campaigns, drawing data-driven
          conclusions alongside a Marketing team.
        </p>
      </section>

      <section className="category-section" id="education">
        <header className="category-header" data-aos="fade-up">
          <h1 className="category-title">Education</h1>
          <hr />
        </header>

        <div data-aos="fade-up">
          <h2>Thinkful</h2>
          <h3>Web Development</h3>
          <p>2019&ndash;</p>
        </div>

        <div data-aos="fade-up">
          <h2>George Fox University</h2>
          <h3>BA in Cinema Media Communications</h3>
          <p>Audio Engineering concentration</p>
          <p>Excellence in Sound Design Award: BEA Las Vegas '14</p>
          <p>2009&ndash;2013</p>
        </div>
      </section>

      <section className="category-section" id="signature" data-aos="fade-up">
        <hr />
        <br />
        <p>
          <a
            href="https://www.linkedin.com/in/austinhuelsbeck"
            title="LinkedIn Profile & Resume"
            target="_blank"
          >
            <strong>Connect &amp; view more details on LinkedIn &rarr;</strong>
          </a>
        </p>
        <br />
        <p className="tiny-text">Updated Jan 19, 2021</p>
      </section>
    </div>
  );
}
